...
#define GAP_ADTYPE_ADV_IND				0x00	// Connectable undirected advertisment 
#define GAP_ADTYPE_ADV_HDC_DIRECT_IND	0x01	// Connectable high duty cycle direcied advertisment 
#define GAP_ADTYPE_ADV_SCAN_IND			0x02	// Scannable undirected advertisment
#define GAP_ADTYPE_ADV_NOCONN_IND		0x03	// Non-Connectable undirected advertisment 
#define GAP_ADTYPE_ADV_LDC_DIRECT_IND	0x04	// Connectable low duty cycle direcied advertisment 
...
/**
 * @brief Set link page timeout
 * @param timeout T = N * 0.625 ms
 */
void gap_set_default_page_timeout(uint16_t timeout);
 ...
 */
void gap_set_scan_parameters(uint8_t scan_type, uint16_t scan_interval, uint16_t scan_window,uint8_t filter_policy,uint8_t filter_duplicates);
 ...
 * @param adv_int_min, Time = N * 0.625ms
 * @param adv_int_max,Time = N * 0.625ms
 * @param adv_type,
...
// Classic
void gap_inquiry_scan_set_param(uint16_t interval,uint16_t window);

void gap_page_scan_set_param(uint16_t interval,uint16_t window);
...
/**
 * @brief set ble device addr
 */
void gap_set_ble_bd_addr(uint8_t *addr);

/**
 * @brief set spp device addr
 */
void gap_set_spp_bd_addr(uint8_t *addr);
...
/**
 * @brief Write Link Policy
 * @param con_handle
 * @param policy
 @ @return 0 if ok
 */
uint8_t gap_write_link_policy(hci_con_handle_t con_handle, uint16_t policy);
/**
 ...
 * @param role :0 master, 1 slave
 @ @return 0 if ok
 */
uint8_t gap_role_switch(hci_con_handle_t con_handle, bd_addr_t addr, uint8_t role);

// LE

/**
 * @brief aes128 start
 * @param type 0:encryption 1:decryption
 * @param aes_encryption_complete_cb
 * @param aes_decryption_complete_cb
 * @param key
 * @param plaintext
 */
void gap_crypto_aes128_start(uint8_t type,void (*aes_encryption_complete_cb)(const uint8_t * data),void (*aes_decryption_complete_cb)(const uint8_t * data),const sm_key_t key, const sm_key_t plaintext);
 ...
 * @param tx_time
 @ @return -1 if err
 */
int gap_le_set_data_length(uint16_t con_handle,uint16_t tx_octets, uint16_t tx_time);
...
uint8_t l2cap_create_channel(host_stack_packet_handler_t packet_handler,bd_addr_t address,uint16_t psm,uint16_t mtu,uint16_t *out_local_cid);
