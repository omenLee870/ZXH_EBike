...
static inline u8_t crc8_ccitt(u8_t val, const void *buf, size_t cnt)
{
	int i;
	const u8_t *p = buf;
			     ...
			     size_t len)
{
	...
	if (blen > 0) {
		rc = fs->flash_write(offset, data8, blen);
		if (rc) {
			/* flash write error */
			goto end;
		}
			 ...
			 size_t len)
{
	...
	rc = fs->flash_read(offset, data, len);
	return rc;
...
static int nvs_flash_erase_sector(struct nvs_fs *fs, u32_t addr)
{
	...
	rc = fs->flash_erase(offset, fs->sector_size);
	if (rc) {
		/* flash erase error */
		return rc;
	}
...
static int nvs_ate_crc8_check(const struct nvs_ate *entry)
{
	...
	crc8 = crc8_ccitt(0xff, entry, offsetof(struct nvs_ate, crc8));
	if (crc8 == entry->crc8 && entry->offset < 0xFF8) {
		return 0;
	}
	return 1;
}
...
static int nvs_prev_ate(struct nvs_fs *fs, u32_t *addr, struct nvs_ate *ate)
{
	...
	*addr += ate_size;
	...
	if (!nvs_ate_crc8_check(&close_ate) && !(close_ate.offset & 0x7)) {
...
static int nvs_gc(struct nvs_fs *fs)
{
	...
	int ret = nvs_ate_crc8_check(&close_ate);
	if (ret || (close_ate.offset & 0x7)){
	...
	while (1) {
		...
		if(nvs_ate_crc8_check(&gc_ate) == 0){
			wlk_addr = fs->ate_wra;
			do{
...
static int nvs_startup(struct nvs_fs *fs)
{
	...
	if (closed_sectors == fs->sector_count) {
		...
		for (u16_t i = 0; i < fs->sector_count; i++) {
			erase_addr = i << ADDR_SECT_SHIFT;
			rc = nvs_flash_erase_sector(fs, erase_addr);
			if (rc) {
				return rc;
			}
		}
		addr = (2 << ADDR_SECT_SHIFT) + fs->sector_size - ate_size;
	}
...
int nvs_read_hist(struct nvs_fs *fs, u16_t id, void *data, size_t len,
		      u16_t cnt)
{
	int rc;
	u32_t wlk_addr, rd_addr;
